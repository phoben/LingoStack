...
> 版本：1.5 · 日期：2026-08-30 · 状态：V1 实施中（核心子任务 3/4 完成）
>
> 变更：
> - v1.5 更新 V1 交付进度：三项核心功能子任务完成，Windows Tauri E2E / CI 基础已合入，最终质量与开源门禁待完成
> - v1.4 固化 V1 主窗口划词流程，并将 IT 词条解释融合进单次翻译请求与翻译结果 tag
...
**译栈（LingoStack）** 是一款面向程序员的跨平台桌面翻译工具，定位为"轻量、简洁、现代"的系统常驻小工具。以**划词翻译**和**写作输出辅助**为核心场景，提供文本翻译、翻译结果中的专业 IT 词条解释、变量名生成、朗读、收藏管理、文档翻译等能力。
...
- **任意应用中**选中文本后通过全局热键唤起主窗口，即可获得专业翻译
...
### 场景 1：划词翻译（P0，核心）
用户在任意应用（IDE、浏览器、文档）中选中文本 → 按划词翻译热键 → 主窗口被唤起并切到翻译视图 → 自动填入原文并流式渲染译文与专业 IT 词条 tag → 可继续朗读/收藏/复制。独立 PopClip 工具栏与翻译浮窗仅作为 V2 备选。

### 场景 2：文本翻译（P0）
主窗口「翻译」标签页：粘贴或输入文本 → 选择源语言/目标语言（默认按语言映射规则自动判定）→ 单次 LLM 请求流式返回译文及最多 5 个上下文专业 IT 词条 → 译文完成后以轻量 tag 展示，hover 或键盘 focus 查看原文语种解释 → 可朗读、收藏、复制。
...
### 场景 7：翻译词条解释（P0）
词条解释与翻译融合为一次模型请求，不提供独立解释模式或第二次请求。应用只提取科技概念、编程/技术栈术语和产品名称等专业实体，普通单词不进入 tag；每次最多 5 项，解释使用原文语种并结合本次翻译语境。无符合条件的词条时不显示空区域。
...
> **V1 决策（2026-08）**：划词翻译改走主窗口，不另建独立浮窗 / 工具栏——少
> 维护独立窗口的代码、状态同步与跨窗口通信。全局热键触发时唤起主窗口、切到
> 翻译视图、取词、填充原文并自动翻译。独立浮窗 / 工具栏列为 V2 备选。

| 窗口 | 触发 | 生命周期 |
|------|------|---------|
| 主窗口 | 托盘 / 热键唤起；划词翻译经热键切入翻译视图 | 关闭即销毁，按需重建 |
| ~~翻译浮窗~~ | ~~全局热键 / 划词「翻译」~~ | V2 备选（见 §12.3） |
| ~~划词工具栏~~ | ~~选中文本自动弹出~~ | V2 备选（见 §7.3 / §12.2） |
...
2. 未命中映射且原文语言与界面语言不同 → 用界面语言作为目标
3. 原文语言与界面语言相同且非英文 → 目标改为英文
4. 原文与界面语言同为英文 → 用全局默认目标语言
```
...
- 翻译自定义 Prompt 只改变内容风格；应用始终追加不可覆盖的译文/词条机器协议
...
> **V1 决策（2026-08）**：不采用 PopClip 式独立工具栏。划词翻译经全局热键
> （默认 `Ctrl+Shift+D`）唤起主窗口的翻译视图，自动取词、填充并翻译；朗读 /
> 收藏 / 复制在翻译视图结果区承接。独立工具栏列为 V2 备选。
...
> V1 不采用——划词翻译走主窗口翻译视图（见 §4.3 / §7.3）。以下为 V2 备选设计。
...
> V1 不采用——翻译在主窗口翻译视图承接。以下为 V2 备选设计。
...
### V1（核心 MVP）
- 划词翻译（热键唤起主窗口 + 读选中；独立 PopClip 工具栏/浮窗后移 V2）
- 文本翻译（单请求融合最多 5 个专业 IT 词条解释）、变量名生成、朗读、收藏管理
- 托盘 + 全局热键 + 冲突检测
...
> **总体判定：核心 MVP 父任务已有 3/4 个子任务完成。** 功能主链路与 Windows 桌面 E2E 基础已交付，但最终质量、开源合规和跨子任务验收尚未完成，因此当前仍是“V1 实施中”，不视为 V1 已完成或可发布。

- [x] **智能翻译契约与可靠性**：单请求流式译文 + 最多 5 个专业 IT 词条、语言规则、零输出单次重试、429 冷却与变量命名五候选边界已实现并归档。
- [x] **设置、本地化与热键**：多提供商设置、主题与中英文持久化、热键冲突/恢复链路已实现并归档。
- [x] **桌面闭环与结果动作**：Windows 划词唤起主窗口、剪贴板降级、托盘/单实例、复制/收藏/朗读/停止等结果动作已实现，Windows 验证证据已记录并归档。
- [x] **桌面 E2E / CI 配套基础**：真实 Tauri 窗口的 WebdriverIO 场景、测试专用隔离和 Windows 阻塞 CI job 已通过 PR #18 合入并归档；macOS / Linux 原生 E2E 仍未实跑。
- [ ] **V1 质量与开源门禁**：仍需补齐跨 IPC、收藏与设置回归，Windows 系统级手工报告，延迟/内存/CPU 测量，`THIRD_PARTY_NOTICES` 确定性生成及最终全量/三平台门禁。
- [ ] **父任务最终验收**：质量门禁完成后，再执行跨子任务验收、最终 Windows 运行证据复核，并归档 V1 父任务。

Issue #2 的 UI 体验优化和 Windows 朗读无声修复也已合入并归档，作为当前 V1 基线维护成果，不计入上述 4 个核心子任务。
