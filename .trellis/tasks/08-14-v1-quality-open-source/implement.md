...
- [ ] 6. 执行 Windows 手工场景、可听 SAPI/原生托盘确认、延迟和资源测量，将证据写入任务目录。（`manual-validation.md` 已有外部划词、剪贴板降级、单实例和两次资源实测；SAPI 实际可听性、原生托盘菜单操作需人工确认，付费 provider HTTP 402 阻断正常网络/常用模型下的延迟测量。）
- [ ] 7. 运行最终全量本地门禁和受支持平台 CI，修复回归并提交，再进入父任务跨子任务验收。（2026-08-30 全量本地门禁再次通过。`origin/develop` `df7591a` 的 CI 33296833111 与 Audit 33296833135 均因账号计费/额度在 runner 调度前失败，所有 job `steps=[]`、`runner_id=0`，故三平台远程门禁尚无可用执行证据；未重跑或修改 GitHub。）
