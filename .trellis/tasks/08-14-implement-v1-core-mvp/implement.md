...
- [x] 4. 全量门禁通过并提交子任务 2。
- [x] 5. 启动并完成 `08-14-v1-desktop-result-actions`：补齐划词反馈、托盘路由、TTS 停止和结果动作。
- [x] 6. 全量门禁通过并提交子任务 3，并记录 Windows UIA、剪贴板降级、SAPI 接受/停止、窗口与单实例证据；可听声音与原生托盘逐项点击留给最终质量任务确认。
- [ ] 7. `tauri-e2e-ci` 已经 PR #18 合入并归档；启动 `08-14-v1-quality-open-source`，复用既有 E2E 基础补齐最终场景、契约测试、许可证通知和集成门禁。
